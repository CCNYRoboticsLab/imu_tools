...
ImuFilterMadgwickRos::ImuFilterMadgwickRos(const rclcpp::NodeOptions &options)
  ...
  if (use_mag_) {
    if (use_magnetic_field_msg_) {
      ...
      vector_mag_subscriber_->registerCallback(&ImuFilterMadgwickRos::imuMagVectorCallback, this);
    }
...
void ImuFilterMadgwickRos::imuCallback(const ImuMsg::SharedPtr imu_msg_raw) {
  ...
  if (!stateless_)
    filter_.madgwickAHRSupdateIMU(ang_vel.x, ang_vel.y, ang_vel.z, lin_acc.x, lin_acc.y, lin_acc.z,
                                  dt);
